...
drivers_plot <- ggplot(drivers_counts, aes(fill=drivers, y=count, x=commodity)) + 
  geom_bar(position="fill", stat="identity") +scale_fill_manual(values=sample(colors))+theme(legend.position = "none")
...
investment_counts <- investment %>% 
  ...
  summarise(count=n()) %>% 
  mutate(fraction = count / sum(count))
investment_counts$investment <- gsub(investment_counts$investment, pattern = "_4/invest_131/commercialisation", replacement = "Support of the FO for commercialisation")
investment_counts$investment <- gsub(investment_counts$investment, pattern = "_4/invest_131/diversification", replacement = "To diversify my income sources") 
investment_counts$investment <- gsub(investment_counts$investment, pattern = "_4/invest_131/income", replacement = "It is the only way for me to generate  an income")           
investment_counts$investment <- gsub(investment_counts$investment, pattern = "_4/invest_131/processing", replacement = "Support of the FO for processing")       
investment_counts$investment <- gsub(investment_counts$investment, pattern = "_4/invest_131/production", replacement = "Support of the FO for production")       
investment_counts$investment <- gsub(investment_counts$investment, pattern = "_4/invest_131/profitability", replacement = "Profitability of the crop")    
investment_counts$investment <- gsub(investment_counts$investment, pattern = "_4/invest_131/vulnerability", replacement = "To reduce vulnerability from climate change")
